...
const AiAssistant: React.FC<AiAssistantProps> = ({ onAddToCart }) => {
  ...
  return (
    <>
      ...
      <div 
        className={`fixed bottom-0 right-0 z-50 w-full sm:w-[400px] sm:bottom-6 sm:right-6 bg-white sm:rounded-3xl shadow-2xl flex flex-col transition-all duration-300 origin-bottom-right overflow-hidden ${
          isOpen ? 'h-full sm:h-[600px] md:h-[650px] opacity-100 scale-100' : 'h-0 opacity-0 scale-90 pointer-events-none'
        }`}
      >
        <div className="bg-orange-600 p-4 flex justify-between items-center text-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-sm tracking-tight">Paw AI Assistant</h3>
              <p className="text-[10px] text-orange-100 uppercase font-bold tracking-widest flex items-center gap-1">
          ...
          {messages.map((msg, idx) => (
            ...
            >
              <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-1 ${
                msg.role === 'user' ? 'bg-slate-300' : 'bg-orange-100 text-orange-600'
              }`}>
              ...
              <div className={`max-w-[85%] space-y-2`}>
                ...
                {msg.generatedImage && (
                  <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm mt-2 bg-white">
                    <img 
                      src={msg.generatedImage} 
                      alt="AI Generated" 
                      className="w-full h-auto object-cover"
                    />
                    <div className="bg-slate-100 px-3 py-1.5 text-[10px] text-slate-500 flex items-center gap-1 font-bold">
                ...
                {msg.recommendedProductIds && msg.recommendedProductIds.length > 0 && (
                  <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide snap-x mt-2">
                    {getRecommendedProducts(msg.recommendedProductIds).map(product => (
                      <div key={product.id} className="min-w-[150px] w-[150px] bg-white p-2 rounded-2xl border border-slate-200 shadow-sm snap-start">
                        <img src={product.image} alt={product.name} className="w-full h-24 object-cover rounded-xl mb-2" />
                        <p className="text-[10px] font-bold text-slate-800 line-clamp-1 mb-0.5">{product.name}</p>
                        <p className="text-xs text-orange-600 font-extrabold mb-2">${product.price}</p>
                        <button 
                          onClick={() => onAddToCart(product)}
                          className="w-full bg-slate-900 text-white text-[10px] font-bold py-1.5 rounded-lg hover:bg-orange-600 transition-colors"
                        >
                          Add to Cart
                        </button>
                      </div>
                 ...
                 <div className="flex items-center gap-2">
                   {isGeneratingImage ? (
                     <>
                      ...
                      <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider animate-pulse">Painting...</span>
                     </>
                   ) : (
        ...
        <div className="p-3 bg-white border-t border-slate-100 shrink-0 pb-safe">
          ...
          >
            <input
              ...
              placeholder="Ask for images or advice..."
              className="flex-1 bg-slate-100 border-0 rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-orange-500 focus:bg-white transition-all outline-none"
            />
            <button 
              type="submit"
              disabled={!input.trim() || isLoading}
              className="p-3 bg-orange-600 text-white rounded-2xl hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md active:scale-95"
            >
              <Send size={18} />
            </button>
          </form>
